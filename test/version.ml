...
let tests = [
  "version", [
    ...
    "hex", `Quick, (fun () ->
      Alcotest.(check int) "number" 0x012200 Luv.Version.hex);

    "version", `Quick, (fun () ->
      Alcotest.(check int) "number" 0x012200 (Luv.Version.version ()));

    "string", `Quick, (fun () ->
      Alcotest.(check string) "value" "1.34.0" (Luv.Version.string ()));
  ]
]
